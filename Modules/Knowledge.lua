...
function M:RefreshProfessions()
    ...
    for i = 1, 5 do
        ...
        if idx then
            ...
            b:SetScript("OnEnter", function(s)
                SP:AnchorTooltipOutsidePanel(GameTooltip, s); GameTooltip:SetText(s._tip); GameTooltip:Show()
            end)
            b:ClearAllPoints()
    ...
    if not self.fireBtn and not InCombatLockdown() then
        ...
        fb:SetScript("OnEnter", function(s)
            SP:AnchorTooltipOutsidePanel(GameTooltip, s); GameTooltip:SetText("Feu de camp"); GameTooltip:Show()
        end)
